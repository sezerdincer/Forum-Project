...
func HandleProfile(w http.ResponseWriter, r *http.Request) {
	...
	err = db.QueryRow("SELECT id, username, email, is_admin FROM users WHERE id = ?", userID).Scan(&user.ID, &user.Username, &user.Email, &user.IsAdmin)
	if err != nil {
		http.Error(w, "User not found", http.StatusInternalServerError)
		return
	}
	err = db.QueryRow("SELECT id, username, email, is_moderator FROM users WHERE id = ?", userID).Scan(&user.ID, &user.Username, &user.Email, &user.IsModerator)
	if err != nil {
		http.Error(w, "User not found", http.StatusInternalServerError)
		return
	}
